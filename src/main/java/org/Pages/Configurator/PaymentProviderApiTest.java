...
/**
 * ✅ Payment Provider API Tests
 * ------------------------------
 * - Logs in (sa / 123456)
 * - Gets JWT token
 * - Tests:
 *   1. GET /assets/locale/en-US.json
 *   2. GET /api/v1/payment-providers?active=true
 *   3. GET /api/v1/cashier-psps
...
public class PaymentProviderApiTest {
    ...
    private static final String PAYMENT_PROVIDERS_URL = "https://pay-app-oilbv.ondigitalocean.app/api/v1/payment-providers?active=true";
    ...
    /** 💳 Payment Providers (active only) */
    @Test
    public void testPaymentProvidersAPI() throws IOException {
        String token = loginAndGetToken();
        int status = sendGetRequest(PAYMENT_PROVIDERS_URL, token, true);
        Assert.assertEquals("Expected 200 for Payment Providers API", 200, status);
    }

    /** 🏧 Cashier PSPs */
    @Test
    public void testCashierPspsAPI() throws IOException {
        String token = loginAndGetToken();
        int status = sendGetRequest(CASHIER_PSP_URL, token, true);
        Assert.assertEquals("Expected 200 for Cashier PSPs API", 200, status);
    }
}
