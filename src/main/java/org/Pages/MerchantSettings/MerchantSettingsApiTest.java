package org.Pages.MerchantSettings;
...
/**
 * ✅ Merchant Settings API Tests
 * -------------------------------
 * - Logs in (sa / 123456)
 * - Gets JWT token
 * - Tests:
 *   1. GET /assets/locale/en-US.json (no auth)
 *   2. GET /api/v1/merchants/current (auth)
 *   3. GET /api/v1/merchants/1/branding (auth)
 * - Verifies HTTP 200 responses
 */
public class MerchantSettingsApiTest {
    ...
    private static final String MERCHANT_CURRENT_URL = "https://pay-app-oilbv.ondigitalocean.app/api/v1/merchants/current";
    private static final String MERCHANT_BRANDING_URL = "https://pay-app-oilbv.ondigitalocean.app/api/v1/merchants/1/branding";
    ...
    /** 🧾 Merchant Current Info */
    @Test
    public void testMerchantCurrentAPI() throws IOException {
        String token = loginAndGetToken();
        int status = sendGetRequest(MERCHANT_CURRENT_URL, token, true);
        Assert.assertEquals("Expected 200 for Merchant Current API", 200, status);
    }

    /** 🎨 Merchant Branding */
    @Test
    public void testMerchantBrandingAPI() throws IOException {
        String token = loginAndGetToken();
        int status = sendGetRequest(MERCHANT_BRANDING_URL, token, true);
        Assert.assertEquals("Expected 200 for Merchant Branding API", 200, status);
    }
}
